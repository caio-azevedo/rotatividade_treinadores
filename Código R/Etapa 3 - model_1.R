...
model_naive1<-formula(pts ~ home_match + pos_adv_rodada + periodo + factor(temporada))
naive1<-lm(model_naive1, data = dados.naive)

model_naive2<-formula(diff_gols ~ home_match + pos_adv_rodada + periodo+ factor(temporada))
naive2<-lm(model_naive2, data = dados.naive)

model_naive3<-formula(vit ~ home_match + pos_adv_rodada + periodo+ factor(temporada))
